...
def is_mel(name, email):
    ...
    Checks to see if user's name or email matches Mel's.
...
def most_and_least_common_type(treats):
    ...
    Expected behavior:
        ...
        >>> example_list = [{'who': 'Heather', 'type': 'dessert'}, {'who': 'Joel', 'type': 'dessert'}, {'who': 'Meggie', 'type': 'appetizer'}]
        >>> most_and_least_common_type(example_list)
        ('dessert', 'appetizer')

    If there's no most OR least common type - all types equally represented (tie) - function selects
    first type and displays for most and least common.

        >>> e_list = [{'who': 'Joel', 'type': 'dessert'}, {'who': 'Meggie', 'type': 'appetizer'}]
        >>> most_and_least_common_type(e_list)
        ('dessert', 'dessert')

    If there's only one type, displays for most & least common:

        >>> e_list2 = [{'who': 'Heather', 'type': 'dessert'}, {'who': 'Joel', 'type': 'dessert'}]
        >>> most_and_least_common_type(e_list2)
        ('dessert', 'dessert')

    If any empty list is given, returns None for both most & least common:

        >>> e_list3 = []
        >>> most_and_least_common_type(e_list3)
        (None, None)
